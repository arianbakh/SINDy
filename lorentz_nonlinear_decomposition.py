...
def _get_theta(x):
    ...
    theta_columns += [np.log(x[:time_frames, i]) for i in range(x.shape[1])]
...
def run():
    ...
    xi = _optimum_sindy(np.log(x_dot ** 2), theta, np.log(x_dot_cv ** 2), theta_cv)
    print(xi)
